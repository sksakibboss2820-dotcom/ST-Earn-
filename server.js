...
async function getSettings() {
  ...
  settings.referral_reward = Number(settings.referral_reward || 0);
  settings.minimum_withdraw = Number(settings.minimum_withdraw || 0);
  settings.withdraw_fee = Number(settings.withdraw_fee || 0);
...
function themeData(settings, user) {
  let themeName = settings.global_theme || "gold";
...
app.get("/api/config", async (req, res) => {
  try {
    ...
    res.json({
      ...
      allowUserTheme: settings.allow_user_theme,
      themes: THEMES
    });
  } catch (error) {
    console.error(error);
...
app.get("/api/me", authMiddleware, async (req, res) => {
  ...
  res.json({
    ...
    user: {
      ...
      tasksDone: user.tasks_done,
      referrals: user.referrals,
      blocked: user.blocked,
      theme: user.theme || ""
    },

    themeData: themeData(settings, user)
  });
});
...
app.post("/api/withdrawals", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const fee = settings.withdraw_fee;
...
app.put("/api/admin/settings", adminMiddleware, async (req, res) => {
  ...
  if (
    values.global_theme &&
    !THEMES[String(values.global_theme)]
  ) {
    return res.status(400).json({
      error: "Invalid theme."
    });
...
app.post("/api/admin/tasks", adminMiddleware, async (req, res) => {
  ...
  const taskType = String(req.body.task_type || "custom");
...
app.put(
  ...
  async (req, res) => {
    ...
    if (req.body.blocked !== undefined) {
      values.push(Boolean(req.body.blocked));
      updates.push(`blocked=$${values.length}`);
    }
...
async function setupTelegramMenuButton() {
  ...
  try {
    ...
    console.log(
      ...
      data.ok ? "configured" : data.description
    );
  } catch (error) {
    console.error(
